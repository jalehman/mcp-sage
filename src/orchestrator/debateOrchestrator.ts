...
import { DebateOptions, DebateResult, DebateConfig, DebateMeta, DebateWarning, DebateLog, ToolType } from '../types/public';
 ...
 */
function parseModelResponse(response: string, promptLength: number): ModelResponse {
 ...
 */
export async function runDebate(options: DebateOptions, sendNotification: NotificationFn): Promise<DebateResult> {
  ...
  const config: Required<DebateConfig> = {
    ...
    rounds: options.debateConfig?.rounds ?? strategy.configDefaults?.rounds ?? 3,
    strategy: options.debateConfig?.strategy ?? options.toolType,
    maxTotalTokens: options.debateConfig?.maxTotalTokens ?? 0,
    logLevel: options.debateConfig?.logLevel ?? strategy.configDefaults?.logLevel ?? 'warn'
  };
  ...
  const timePhase = async <T>(phaseName: string, func: () => Promise<T>): Promise<T> => {
  ...
  const addWarning = (code: DebateWarning['code'], message: string, phase: DebateWarning['phase']) => {
  ...
  const tokenBudget = config.maxTotalTokens > 0 ? createTokenBudget(config.maxTotalTokens) : null;
  ...
  if (debateModels.length === 1) {
    ...
    try {
      ...
      const response = await timePhase<ModelResponse>('generate', async () => {
        const rawResponse = await sendToModelWithFallback(
          generatePrompt,
          { modelName, modelType: modelName.includes('gemini') ? 'gemini' : 'openai', tokenCount: generatePrompt.length / 4 },
          sendNotification
        );
      ...
      addWarning('GEN_FAIL', `Generation failed: ${error instanceof Error ? error.message : String(error)}`, 'generate');
    ...
    for (let round = 1; round <= config.rounds; round++) {
      ...
      if (tokenBudget && !tokenBudget.beginRound(50000)) { // Rough estimate
        addWarning('TOKEN_BUDGET', 'Token budget exceeded. Ending debate early.', 'generate');
        break;
      }
      ...
      if (round === 1) {
        ...
        const generationTasks = modelIds.map(modelId => {
          ...
          return async () => {
            ...
            // Record the prompt in the transcript
            addTranscript(`[GENERATE MODEL ${modelId}]\nPrompt:\n${generatePrompt}\n`);
            ...
            try {
              ...
              if (process.env.SKIP_API_CALLS === "true") {
                ...
                const response = parseModelResponse(mockResponse, generatePrompt.length);
                ...
                addTranscript(`[GENERATE RESPONSE MODEL ${modelId} (MOCK)]\n${response.text}\n`);
                ...
                const rawResponse = await sendToModelWithFallback(
                  generatePrompt,
                  { modelName, modelType, tokenCount: generatePrompt.length / 4 },
                  sendNotification
                );
                
                const response = parseModelResponse(rawResponse, generatePrompt.length);
                ...
                // Record the response in the transcript
                addTranscript(`[GENERATE RESPONSE MODEL ${modelId}]\n${response.text}\n`);
              ...
              addWarning('GEN_FAIL', `Generation failed for model ${modelId}: ${error instanceof Error ? error.message : String(error)}`, 'generate');
              return null;
            }
          };
        });
        ...
        debateContext.candidates = candidateResults.filter((r): r is string => r !== null);
        
        if (debateContext.candidates.length === 0) {
          throw new Error('All model generations failed. Cannot continue debate.');
        }
      }
      ...
      if (round < config.rounds) {
        ...
        console.error(`[DEBUG-Debate] Starting critique phase with candidates: ${debateContext.candidates.length}`);
        
        const modelIds = Object.keys(idToModel);
        console.error(`[DEBUG-Debate] Model IDs for critique: ${modelIds.join(', ')}`);
        
        const critiqueTasks = modelIds.map(modelId => {
          const modelName = idToModel[modelId];
          const modelType = modelName.includes('gemini') ? 'gemini' : 'openai';
          console.error(`[DEBUG-Debate] Creating critique task for model ID: ${modelId}, name: ${modelName}, type: ${modelType}`);
          
          return async () => {
            console.error(`[DEBUG-Debate] Starting critique task for model ${modelId} (${modelName})`);
            ...
            console.error(`[DEBUG-Debate] Critique prompt generated for model ${modelId}, length: ${critiquePrompt.length}`);
            
            // Record the prompt in the transcript
            addTranscript(`[CRITIQUE MODEL ${modelId}]\nPrompt:\n${critiquePrompt}\n`);
            ...
            try {
              ...
              if (process.env.SKIP_API_CALLS === "true") {
                ...
                const response = parseModelResponse(mockResponse, critiquePrompt.length);
                ...
                // Record the mock response in the transcript
                addTranscript(`[CRITIQUE RESPONSE MODEL ${modelId} (MOCK)]\n${response.text}\n`);
                ...
                // Make actual API call
                console.error(`[DEBUG-Debate] Sending critique prompt to model ${modelId} (${modelName})`);
                
                console.error(`[DEBUG-Debate] About to call sendToModelWithFallback for ${modelName}`);
                let response;
                
                try {
                  const rawResponse = await sendToModelWithFallback(
                    critiquePrompt,
                    { modelName, modelType, tokenCount: critiquePrompt.length / 4 },
                    sendNotification
                  );
                  
                  console.error(`[DEBUG-Debate] Received response from ${modelName} for critique`);
                  response = parseModelResponse(rawResponse, critiquePrompt.length);
                  console.error(`[DEBUG-Debate] Parsed response, length: ${response.text.length}`);
                } catch (critErr) {
                  console.error(`[DEBUG-Debate] ERROR in critique API call: ${critErr instanceof Error ? critErr.message : String(critErr)}`);
                  throw critErr;
                }
                ...
                // Record the response in the transcript
                addTranscript(`[CRITIQUE RESPONSE MODEL ${modelId}]\n${response.text}\n`);
              ...
              addWarning('GEN_FAIL', `Critique failed for model ${modelId}: ${error instanceof Error ? error.message : String(error)}`, 'critique');
              return null;
            }
          };
        });
        ...
        debateContext.critiques = critiqueResults.filter((r): r is string => r !== null);
      }
    }
    ...
    const judgeModelType = judgeModelName.includes('gemini') ? 'gemini' : 'openai';
    ...
    try {
      ...
      const judgeResponse = await timePhase<ModelResponse>('judge', async () => {
        // Check if we're in test mode with API calls skipped
        if (process.env.SKIP_API_CALLS === "true") {
          // Create a mock response for testing
          ...
          const rawResponse = await sendToModelWithFallback(
            judgePrompt,
            { modelName: judgeModelName, modelType: judgeModelType, tokenCount: judgePrompt.length / 4 },
            sendNotification
          );
      ...
      const judgeResult = strategy.parseJudge(judgeResponse.text, debateContext.candidates);
      ...
      addWarning('JUDGE_MALFORMED', `Judge phase error: ${error instanceof Error ? error.message : String(error)}`, 'judge');
      addFallback('judge', `Error: ${error instanceof Error ? error.message : String(error)}`);
  ...
  switch (options.toolType) {
    case ToolType.Plan:
      (result as any).finalPlan = finalOutput;
      break;
