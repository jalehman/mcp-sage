...
export async function sendGeminiPrompt(
  ...
  if (notifyFn) {
    await notifyFn({
      level: "info",
      data: `Starting sendGeminiPrompt with model: ${model}`,
    });
  }
...
program
  ...
  .option(
    "-m, --model <name>",
    "Specify the model to use (default: gemini-2.5-pro-preview-03-25)",
  )
  .option(
    ...
    try {
      ...
      if (!prompt) {
        ...
        if (!process.stdin.isTTY) {
          ...
          console.error(
            "Error: No prompt provided. Please provide a prompt as an argument, from a file, or via stdin.",
          );
          process.exit(1);
        }
      }
