      ...
      if (response.result && response.result.content) {
        ...
        const opinionRequest = {
          ...
          params: {
            ...
            arguments: {
              prompt:
                "Is it better to use functional or class components in modern React?",
              paths: [
      ...
      if (response.result && response.result.content) {
        ...
        const reviewRequest = {
          ...
          params: {
            ...
            arguments: {
              instruction:
                "Suggest improvements for error handling in this codebase",
              paths: [
                path.join(__dirname, "..", "src"),
                path.join(__dirname, "..", "test"),
              ],
      ...
      lines.forEach((line) => {
        ...
        try {
          ...
          if (response.id === 1 && response.result) {
            process.stderr.write(
              "Sending sage-plan tool request with debate extension...\n",
            );
            ...
            const toolRequest = {
              ...
              params: {
                ...
                arguments: {
                  prompt:
                    "Create a lightweight state management library for React",
                  paths: [
                    path.join(__dirname, "..", "src"),
                    path.join(__dirname, "..", "test"),
                  ],
          ...
          if (response.method && response.method.startsWith("notifications/")) {
            ...
            if (
              response.params.method &&
              response.params.method.startsWith("notifications/")
            ) {
              process.stderr.write(
                `Notification: ${response.params.params.level} - ${response.params.params.data}\n`,
              );
            } else {
              process.stderr.write(
          ...
          if (
            planHandler(response) ||
            opinionHandler(response) ||
            reviewHandler(response)
          ) {
